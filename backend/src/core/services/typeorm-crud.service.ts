...
export class TypeOrmCrudService<T> extends CrudService<T> {
  ...
  public async createOne(req: CrudRequest, dto: DeepPartial<T>): Promise<T> {
    ...
    const entity = this.prepareEntityBeforeSave(dto, req);
  ...
  public async createMany(
    ...
    const bulk = dto.bulk
      .map(one => this.prepareEntityBeforeSave(one, req))
  ...
  public async updateOne(req: CrudRequest, dto: DeepPartial<T>): Promise<T> {
    ...
    const updated = await this.repo.save(
      plainToClass(this.entityType, toSave, (req as any).transformOptions),
    );
  ...
  protected prepareEntityBeforeSave(dto: DeepPartial<T>, req: CrudRequest): T {
    /* istanbul ignore if */
    if (!isObject(dto)) {
      return undefined;
    }

    if (hasLength(req.parsed.paramsFilter)) {
    ...
    return dto instanceof this.entityType
      ? Object.assign(dto, req.parsed.authPersist)
      : plainToClass(
          this.entityType,
